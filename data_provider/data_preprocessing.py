...
class GasDataUtils:
    ...
    def train_test_split(data, train_ratio=0.8, test_ratio=0.2):
        ...
        assert type(data)==list, "`data` must be a list, but got {}".format(type(data))
        
        n_series = len(data)
        num_train = int(train_ratio*n_series)
        num_test = int(test_ratio*n_series)
        ...
        return (data_train, data_test), (train_series_indices, test_series_indices)
    ...
    def time_series_slice(data, input_len, output_len,
        ...
        def split_2d_np(np_2d, input_len, output_len, overlap=0):
            r"""
            Split a single 2-d numpy array into input and output sequences.

            :param np_2d: a 2-d numpy array of shape: (n_timesteps, n_vars)
            ...
            """
            assert np_2d.ndim==2, "`np_2d` must be a 2-d numpy array, got shape {}".format(np_2d.shape)
...
class GasData:
    ...
    def __init__(self, data, input_len, output_len,
        ...
        r"""
        :param data: list of numpy arrays of shape: (n_timesteps, n_vars). n_timesteps can be different, but n_vars must be the same.
        :param input_len: the temporal length of each input sequence.
        :param output_len: the temporal length of each output sequence.
        ...
        :param overlap: the number of overlapping time steps between input and output sequences. If 0, the input and output sequences are non-overlapping. If 1, the last time step of the input sequence is the same as the first time step of the output sequence.
        
        :param input_indices: a list of indices of the input variables. If None, all variables are used as input variables.
        :param output_indices: a list of indices of the output variables. If None, all variables are used as output variables.
        :param var_names: a list of variable names. If None, the variable names are set to "var_i" for i in range(n_vars).
        :param var_units: a list of variable units. If None, the variable units are set to "unit_i" for i in range(n_vars).

        :param transform_func: a function to transform the data. If None, no transformation is applied. The function takes a numpy array of shape (n_timesteps, n_vars) as input, and returns a transformed numpy array of the same shape.
        :param inverse_transform_func: a function to inverse transform the data. If None, no inverse transformation is applied.
        """
        assert type(data)==list, "parameter `data` must be a list, but got {}".format(type(data))
        ...
        # Set the indices of the input and output variables
        self.input_indices=list(range(self.n_vars)) if input_indices is None else input_indices
        self.output_indices=list(range(self.n_vars)) if output_indices is None else output_indices

        # Set the variable names and units
        ...
        # Other attributes that will be instantiated later
    ...
    def transform(self):
        ...
        `self.transform_func` takes a numpy array of shape (n_timesteps, n_vars) as input, and returns a transformed numpy array of the same shape.
        """
    ...
    def standardize(self):
        ...
        `self.data` is transformed in place.
        """
    ...
    def train_test_split(self, train_ratio=0.8, test_ratio=0.2):
        ...
        `self.data_train`, `self.data_test`, `self.train_indices`, and `self.test_indices` are instantiated.
        """
        (self.data_train, self.data_test), (self.train_indices, self.test_indices)\
            = GasDataUtils.train_test_split(self.data, train_ratio, test_ratio)
    ...
    def time_series_slice(self):
        ...
        self.X_train_grouped, self.Y_train_grouped = GasDataUtils.time_series_slice(self.data_train, self.input_len, self.output_len, self.input_indices, self.output_indices, self.overlap)
        self.X_test_grouped, self.Y_test_grouped = GasDataUtils.time_series_slice(self.data_test, self.input_len, self.output_len, self.input_indices, self.output_indices, self.overlap)
    ...
    def build_train_test_set(self):
        ...
        print("Train and test sets have been built.")
